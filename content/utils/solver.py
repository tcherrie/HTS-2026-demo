...
def newton(fes : ngs.FESpace,                                                       # finite element space
    ...
    inverse : str, optional
        Method to compute the inverse of the tangent matrix. Options are "superlu" (default), "pardiso", "sparsecholesky", or any method supported by ngs.Inverse.
...
def newton_mixed(fes : ngs.FESpace,                                                 # finite element space
          ...
          step_factor_linesearch : float = 0.5, # step size reduction factor in (0, 1) to reduce the step if too big
          use_multithreading : bool = False,     # flag to enable parallelization with TaskManager during assembly
          inverse : str = "superlu"              # method to compute the inverse of the tangent matrix
          ) -> dict:
