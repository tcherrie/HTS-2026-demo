...
H_s_dir = Bmax / mu0 * CF((cos(theta), sin(theta)))
...
fes = HCurl(mesh, order=1, dirichlet="out")  # Functional space, order and boundary conditions
...
while t_current < T_final:
    ...
    while not converged:
        ...
        result = newton( fes, residual, d_residual, sol, verbosity=1, use_multithreading = False )
...
plot_result(time_list, ac_losses_list, filename_comsol = "results_COMSOL/2D/external_field/AC_Losses_"+str(int(Bmax*1000))+"mT.txt")

## Data save

import numpy as np
np.save('Losses_2D_tape_external_field.npy', np.array(ac_losses_list))     # Instantanuous AC losses 
np.save('Time_2D_tape_external_field.npy', np.array(time_list))            # Time instants  
np.save('Steplist_2D_tape_external_field.npy', np.array(steplist))         # The time steps used
